...
class SteeredModel:
    def __init__(
        ...
        source_module = rgetattr(
            self.layers[self.source_layer_idx], self.source_module_name
        )
    ...
    def train(self, examples, num_vectors):
        ...
        self.learned_vectors = torch.zeros(
            self.num_vectors, self.width, device=self.device
        )
        ...
        for ex in examples:
            ...
            with torch.no_grad():
                ...
                self.unsteered_targets.append(
                    hidden_states[self.target_layer_idx][:, self.target_token_idxs, :]
                )
        ...
        for i in tqdm.tqdm(range(num_vectors), desc="Training steering vectors"):
            ...
            with torch.no_grad():
                if self.orthogonal_vectors:
                    ...
                    rand_vec = project_orthogonal_subspace(
                        rand_vec, self.learned_vectors, self.normalization
                    )
                    self.bias.data = self.normalization * nn.functional.normalize(
                        rand_vec, dim=0
                    )
                else:
                    self.bias.data = self.normalization * nn.functional.normalize(
                        torch.randn(self.width, device=self.device), dim=0
                    )

            optimizer = optim.AdamW(
                [self.bias], lr=0.001, betas=(0.9, 0.98), weight_decay=0.0, amsgrad=True
            )
            ...
            for t in range(self.num_steps):
                ...
                for ex, target_unsteered in zip(examples, self.unsteered_targets):
                    model_inputs = self.tokenizer([ex], return_tensors="pt").to(
                        self.device
                    )
                    if self.target_module == "residual":
                        hidden_states = self.model(
                ...
                if self.orthogonal_vectors:
                    with torch.no_grad():
                        self.bias.grad = project_orthogonal_subspace(
                            self.bias.grad, self.learned_vectors, self.normalization
                        )

                with torch.no_grad():
                    self.bias.grad -= (
                        torch.dot(self.bias.grad, self.bias)
                        * self.bias
...
if __name__ == "__main__":
    ...
    steered_model = SteeredModel(
        model, tokenizer, source_layer_idx=3, target_layer_idx=10
    )
    ...
    perturbed_response = tokenizer.decode(
        perturbed_output_ids[0], skip_special_tokens=True
    )

    steered_model.bias.data = -perturbation_vector
    with torch.no_grad():
        subtracted_output_ids = model.generate(**benign_inputs, max_length=100)
    subtracted_response = tokenizer.decode(
        subtracted_output_ids[0], skip_special_tokens=True
    )
