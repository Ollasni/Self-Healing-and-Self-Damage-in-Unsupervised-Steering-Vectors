# take from backup-research repo from original Self-Repair paper
import torch as t
...
def get_hook_name_filter(model: HookedTransformer):
    ...
    def name_filter(name):
        if name.endswith("_result") and (not model.cfg.use_attn_result):
            return False
        elif any([name.endswith(f"{c}_input") for c in "qkv"]) and (
            not model.cfg.use_split_qkv_input
        ):
            return False
        elif any([name.endswith(f"{c}_normalized_input") for c in "qkv"]) and (
            not model.cfg.use_split_qkv_normalized_input
        ):
            return False
...
class Node:
    ...
    def __init__(
        ...
        if neuron is not None:
            assert isinstance(
                neuron, int
            ), f"neuron should be an int, not {type(neuron)}"
        self.head = head
        self.neuron = neuron
        ...
        if not any(
            name in self.activation_name for name in ["q", "k", "v", "z", "pattern"]
        ):
            assert (
                self.head is None
            ), f"Can't specify `head` for activation {self.activation_name}."

        # Check if neuron dimension is appropriate
        if not any(name in self.activation_name for name in ["pre", "post"]):
            assert (
                self.neuron is None
            ), f"Can't specify `neuron` for activation {self.activation_name}."
    ...
    def get_posn(self):
        ...
        if self.layer is None:
            layer = (
                -1 if self.component_name in ["embed", "pos_embed", "tokens"] else 10e5
            )
        else:
            layer = self.layer
        ...
        if "resid_pre" in self.activation_name:
            ...
            within_layer = 0.1
        elif ".attn." in self.activation_name or any(
            [f"{s}_input" in self.activation_name for s in "qkv"]
        ):
            within_layer = 0.2
    ...
    def check_sender(self, model: HookedTransformer) -> List["Node"]:
        ...
        assert (
            self.seq_pos is None
        ), "Error: can't specify `seq_pos` for path patching. You should use the `seq_pos` argument in the `path_patch` function."

        # Make sure it's actually real
        assert (
            self.activation_name in model.hook_dict.keys()
        ...
        assert (
            self.component_name in valid_sender_nodes
        ), f"Error: node '{self.component_name}' is not a valid sender node. Valid sender nodes are: {valid_sender_nodes}"
    ...
    def check_and_split_receiver(self, model: HookedTransformer) -> List["Node"]:
        ...
        assert (
            self.component_name != "mlp_in"
        ), "Error: 'mlp_in' is not a valid sender node. Please use 'pre' for the output of attention heads (to select specific neurons, use the 'neuron' argument in the Node constructor)."

        # Do main validity check
        valid_receiver_nodes = [
            "q",
            "k",
            "v",
            "pattern",
        ...
        assert (
            self.component_name in valid_receiver_nodes
        ), f"Error: node '{self.component_name}' is not a valid receiver node. Valid receiver nodes are: {valid_receiver_nodes}"
        ...
        if self.component_name == "pattern":
            return [
                Node("q", layer=self.layer, head=self.head),
                Node("k", layer=self.layer, head=self.head),
            ]
        else:
            return [self]
    ...
    def get_patching_hook_fn(
        ...
        def hook_fn(
            activations: Float[Tensor, "..."], hook: HookPoint
        ) -> Float[Tensor, "..."]:
            ...
            if self.head is not None:
                ...
                if "pattern" in hook.name or "score" in hook.name:
                    assert isinstance(
                        seq_pos_indices, slice
                    ), "Can't patch attention patterns/scores at specific sequence positions (ambiguous whether this is query or key positions)."
...
class IterNode:
    ...
    def __init__(
        ...
        self.component_names = (
            [node_names] if isinstance(node_names, str) else node_names
        )
    ...
    def get_node_dict(
        ...
        if self.seq_pos == "each":
            seq_pos_indices = list(
                einops.repeat(t.arange(seq_len), "s -> s b 1", b=batch_size)
            )
        ...
        for node_name, shape_names in self.shape_names.items():
            ...
            self.nodes_dict[node_name] = [
                (args[0], Node(node_name, **kwargs))
                for args, kwargs in shape_values_list
            ]
...
def _path_patch_single(
    ...
    if direct_includes_mlps or any([n.component_name == "pre" for n in receiver_nodes]):
        ...
        if not direct_includes_mlps:
            hooks_for_freezing.append(
                (
                    lambda name: name.endswith("post"),
                    partial(hook_fn_generic_patching, cache=orig_cache),
                )
            )
        ...
        for sender_node in sender_nodes:
            ...
            diff = (
                new_cache[sender_node.activation_name]
            ...
            if sender_node.component_name == "post":
                neuron_slice = (
                    slice(None) if sender_node.neuron is None else [sender_node.neuron]
                )
                diff = einops.einsum(
                    diff[..., neuron_slice],
                    model.W_out[sender_node.layer, neuron_slice],
                    "batch pos d_mlp, d_mlp d_model -> batch pos d_model",
                )
            # If it's the "z" part of attn heads, we map through W_O (maybe just taking one head)
            elif sender_node.component_name == "z":
                head_slice = (
                    slice(None) if sender_node.head is None else [sender_node.head]
                )
                diff = einops.einsum(
                    diff[..., head_slice, :],
        ...
        for sender_node, diff in sender_diffs.items():
            ...
            for i, receiver_node in enumerate(receiver_nodes):
                ...
                if receiver_node.component_name in [
                    "q",
                    "k",
                    "v",
                    "q_input",
                    ...
                    assert (
                        model.cfg.use_split_qkv_input
                    ), "Direct patching (direct_includes_mlps=False) requires use_split_qkv_input=True. Please change your model config."

                    # q/k/v should be converted into q_input/k_input/v_input
                    if receiver_node.component_name in ["q", "k", "v"]:
                        receiver_node = Node(
                            f"{receiver_node.component_name}_input",
                            layer=receiver_node.layer,
                            head=receiver_node.head,
                        )
                ...
                if len(model.hook_dict[receiver_node.activation_name].ctx) == 0:
                    model.hook_dict[receiver_node.activation_name].ctx[
                        "receiver_activations"
                    ] = t.zeros_like(orig_cache[receiver_node.activation_name])
                    model.add_hook(
                        receiver_node.activation_name,
                        partial(
                            hook_fn_generic_patching_from_context,
                            name="receiver_activations",
                            add=True,
                        ),
                        level=1,
                    )
                ...
                if receiver_node.component_name in ["q_input", "k_input", "v_input"]:
                    head_slice = (
                        slice(None)
                        if (receiver_node.head is None)
                        else [receiver_node.head]
                    )
                    # * TODO - why is this needed?
                    if (
                        diff.shape
                        != model.hook_dict[receiver_node.activation_name]
                        .ctx["receiver_activations"][
                            batch_indices, seq_pos_indices, head_slice
                        ]
                        .shape
                    ):
                    ...
                    model.hook_dict[receiver_node.activation_name].ctx[
                        "receiver_activations"
                    ][batch_indices, seq_pos_indices, head_slice] += diff
                ...
                # The remaining case (given that we aren't handling "pre" here) is when receiver is resid_pre/mid/post
                    ...
                    model.hook_dict[receiver_node.activation_name].ctx[
                        "receiver_activations"
                    ][batch_indices, seq_pos_indices] += diff
    ...
    if apply_metric_to_cache:
        ...
        if isinstance(patching_metric, str):
            loss = model(
                orig_input,
                return_type="loss",
                loss_per_token=(patching_metric == "loss_per_token"),
            )
            model.reset_hooks()
            return loss
        else:
...
def path_patch(
    ...
    if isinstance(patching_metric, str):
        assert patching_metric in [
            "loss",
            "loss_per_token",
        ], "Invalid patching_metric argument."
    assert not (
        isinstance(patching_metric, str) and apply_metric_to_cache
    ), "Can't apply metric to cache if metric is 'loss' or 'loss_per_token'."
    ...
    if new_cache == "zero":
        new_cache = ActivationCache(
            {k: t.zeros_like(v) for k, v in orig_cache.items()}, model=model
        )
    ...
    if not any(
        ...
        return path_patch_single(
            sender=sender_nodes, receiver=receiver_nodes, seq_pos=seq_pos
        )
...
def act_patch(
    ...
    ), "Must specify either new_input or new_cache."
    if isinstance(patching_metric, str):
    ...
    if new_cache == "zero":
        _, cache = model.run_with_cache(orig_input, return_type=None)
        new_cache = ActivationCache(
            {k: t.zeros_like(v) for k, v in cache.items()}, model=model
        )
    elif new_cache is None:
    ...
    return {
        node_name: (
            t.tensor(results).reshape(
                list(patching_nodes.shape_values[node_name].values())
            )
            if isinstance(results[0], float)
            else results
        )
